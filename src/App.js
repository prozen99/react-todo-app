import React, { Component } from "react";//리액트 라이브러리 에서 가져옴.
...
export default class App extends Component {
  ...
  getStyle = (completed) =>{ // getStyle은 완료가 된건지 아닌지 completed인지 값을받아줌.
    return {
      padding:"10px",
...
handleClick=(id)=>// 어떤 list를 삭제할지 id를 정해줘야 내가 
...
handleChange=(e)=>
{
  ...
  this.setState({value:e.target.value}); // value 값을 e.target.value 값으로 바꾸어줌.
}

handleSubmit=(e)=> //제출 이벤트를 만드는 곳 . 
{
  e.preventDefault();//form 안에 input을 전송할 때 페이지 리로드 되는 것을 막아준다.

  let newTodo={
    id:Date.now(), // 유니크한 키 생성 
    title:this.state.value, // 상태 값을 저장하기 위한 value 
    completed:false,
  };
...
handleCompleChange=(id)=> // checkbox의 id를 받아서 어떤거를 바꿀건지를 정하는 부분임.
{
      ...
      <div className="container">
        <div className="todoBlock">
          ...
          {this.state.todoData.map(data=>( 
             ...
             />
             {data.title}
             <button style={this.btnsty} onClick={()=>this.handleClick(data.id)}>x</button>
             {
               /*
